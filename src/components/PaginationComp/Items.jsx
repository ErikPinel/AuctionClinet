...
export const Items = ({ posts, loading, setFilter }) => {
  ...
  function del(i,post) {
    ...
    if (post.section == "Men-section")  {axios.delete(`http://localhost:5000/api-itemMen/itemmen/${posts[i]._id}`);}
      else if (post.section == "Women-section") {axios.delete(`http://localhost:5000/api-itemWomen/itemwomen/${posts[i]._id}`);}
       else if (post.section == "Kids-section") {axios.delete(`http://localhost:5000/api-itemKids/itemkids/${posts[i]._id}`);}
  ...
  function addBoughtSoled(post, index)
{
 
 let itemsBought;
 let sellerItem;
  axios
  .post(`http://localhost:5000/api-users/users/logged/`, { id:  localStorage.getItem("logged") })
  .then((data) => {itemsBought=data.itemsBought})

  axios
  .patch(`http://localhost:5000/api-users/users/logged/`, { id: post.SellerID })
  .then((data) => {sellerItem=data.itemsSoled} );
  

  axios .patch(`http://localhost:5000/api-users/users/addBought/${localStorage.getItem("logged")}`, { itemsBought: itemsBought})
  .then((data) => {})
